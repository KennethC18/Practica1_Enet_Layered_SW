...
try:
    while True:
        ...
        # Find the end of the data by removing trailing zeros (padding)
        data_end = payload_len
        while data_end > 0 and payload[data_end - 1] == 0:
            data_end -= 1

        # Extract the CRC integer value from the last 4 bytes before padding
        if data_end < 4:  # Ensure there's enough data for CRC
            print("Payload too short for CRC!")
            continue
        crc_start = data_end - 4
        packet_crc = int.from_bytes(payload[crc_start:data_end], byteorder='little')
        print(f"CRC32: {packet_crc:08x}")

        # Compute the CRC32 of the data (excluding CRC and padding)
        calc_crc = zlib.crc32(payload[:crc_start])
        ...
        # Decrypt the data
        decrypted_data = decrypt(payload[:crc_start], aes_key)
        ...
        calc_crc = zlib.crc32(encrypted_data) & 0xFFFFFFFF
